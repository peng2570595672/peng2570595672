...
Page({
	data: {
		...
		problemList: [
			...
			`},
			{open: false, title: 'Q：我在账单详情里看到了“违约金”，为什么要收取我的违约金。',content: `
				A： 您办理的ETC享受了先通行后付费，当您的通行费欠费后，由我司垫付这笔通行费给高速。若您在欠费后的指定时间内（客车用户为扣款失败第16日，货车用户为扣款失败的当日）仍未缴清的，每日将加收万分之五的违约金。每笔通行费的违约金每年累计金额不超过该通行费金额的18%。为了保证您的正常通行不受影响，请您务必保证您支付账户余额充足。
			`},
