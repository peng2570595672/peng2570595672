...
Page({
		...
		this.transCommand(2, cosArr, (code, res, err) => {
			...
			if (+code === 0) {
				...
				this.setData({
					// IssuerMarking,
					// protocolType,
					version: parseInt(contractVersion,16),
					rand: contractNumber,
					startTime,
					endTime
				});
	...
	getRandomForReadCarInfoFromOBU () {
		...
		this.transCommand(2, cosArr, (code, res) => {
			console.log(res);
			// 获取随机数
	...
	async requestServer (random) {
		...
		// 	content: this.data.carInfoFromOBU,
		// 	picData: app.globalData.hunanPic,
	...
	sendToActivate (res) {
		...
		this.transCommand(2, cosArr, async (code, res, err) => {
	...
	async requestServerForComfirm () {
		...
		let obj = wx.getStorageSync('activate-info');
		obj = JSON.parse(obj);
		// util.ajax('consumer/etc/public/to-confirm-obu', {
