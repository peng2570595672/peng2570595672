...
const bleUtil = require('../../libs/tianjin_artc_sdk/ArtcBleUtil.js');
...
Page({
	...
	onLoad () {
		// // TODO TEST CODE
		// app.globalData.memberId = '631905151791136768';
		// app.globalData.orderInfo.orderId = '631905421702987776';
		// app.globalData.token = 'daa9d4593c6b468cacf33f49f9be8bad';
	...
	getContractInfo () {
		...
		this.sendAndReceive(1, cosArr, (code, res) => {
			let info = res[1];
			if (+code === 0) {
				// 发行方标识
	...
	cardSurfaceNumber () {
		...
		this.sendAndReceive(0, cosArr, (code, res) => {
			console.log('读取卡片信息：');
			console.log(code);
			console.log(res);
				...
				wx.onBluetoothDeviceFound((res) => {
					for (let i = 0; i < res.devices.length; i++) {
						...
						if (!isHave) {
							...
							if (res.devices[i].localName === 'Artc_Wechat') {
	...
	sendAndReceive (code, arr, callback) {
		bleUtil.sendAndReceive(code, arr, {
			success (res) {
				console.log(res);
				callback(0, res.data);
			},
