...
* 青海万集
 */
import wjBleAPI from '../../libs/qinghai_wanji_sdk/wjBleAPI.js';

const util = require('../../../../utils/util.js');
const encodeToGb2312 = require('../../libs/encodeToGb2312.js').encodeToGb2312;
const JLObuSDK = require('../../libs/qinghai_wanji_sdk/MyObuSDK.js');
const app = getApp();
let timer;
Page({
	data: {
	...
	cardSurfaceNumber () {
		...
		JLObuSDK.transCmd(['00A40000023F00', '00A40000021001',
			...
			if (res.code === 0) {
				...
				util.sendException2Server(`青海二发万集：${JSON.stringify(this.data.info)}`, cosArr, -1, res);
			}
		});
	},
	// 从obu中读取信息
	...
	writeDataToObuForSysInfo (res) {
		...
		JLObuSDK.transCmd([res.data.apdu + res.data.mac], '20', (r) => {
			if (r.code === 0) {
				...
				util.sendException2Server(`青海二发万集：${JSON.stringify(this.data.info)}`, cosArr, -1, r);
			}
		});
	},
	// 发送数据到后台确定写入是否成功
