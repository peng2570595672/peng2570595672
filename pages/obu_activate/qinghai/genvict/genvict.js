...
Page({
	...
	cardSurfaceNumber () {
		...
		bleUtil.cosChannel('00', cosArr, (code, res) => {
			if (parseInt(code) === 0) {
				...
				util.sendException2Server(`青海二发金溢：${JSON.stringify(this.data.info)}`, cosArr, code, res);
			}
		});
	},
	// 从obu中读取信息
	getSerialNumberAndVersion () {
		...
		bleUtil.gvEsamChannel('20', cosArr, (code, res) => {
			if (parseInt(code) === 0) {
				...
				// 合同版本
				let version = res[1].slice(18, 20);
				console.log('合同版本:' + version);
				// 截取合同号
				let serialNumber = res[1].slice(20, 36);
	...
	// 写入数据到obu
	writeDataToObuForSysInfo (r) {
		console.log(r.data.apdu + r.data.mac);
		let cosArr = [r.data.apdu + r.data.mac];
		bleUtil.gvEsamChannel('20', cosArr, (code, res) => {
			console.log(code);
			console.log(res);
			if (parseInt(code) === 0) {
				...
				if (result === '9000') {
					...
					this.confirmSysInfo(r.data.apdu, '00', r.data.proListNo);
				} else {
					this.confirmSysInfo(r.data.apdu, '01', r.data.proListNo);
					util.sendException2Server(`青海二发金溢：${JSON.stringify(this.data.info)}`, cosArr, code, res);
				}
			} else {
				this.confirmSysInfo(r.data.apdu, '01', r.data.proListNo);
		...
		bleUtil.gvEsamChannel('20', cosArr, (code, res) => {
			if (parseInt(code) === 0) {
				...
				if (result === '9000') {
					...
					this.confirmAppInfo(r.data.apdu, '00', r.data.proListNo);
				} else {
					this.confirmAppInfo(r.data.apdu, '01', r.data.proListNo);
					util.sendException2Server(`青海二发金溢：${JSON.stringify(this.data.info)}`, cosArr, code, res);
				}
			} else {
				this.confirmAppInfo(r.data.apdu, '01', r.data.proListNo);
	...
	get4BitRandomByPICCFor0015 () {
		...
		bleUtil.cosChannel('00', cosArr, (code, res) => {
			if (parseInt(code) === 0) {
				let carInfo = res[1].substring(0, res[1].length - 4);
				console.log(carInfo);
				let info = res[2];
		...
		bleUtil.cosChannel('00', cosArr, (code, res) => {
			if (parseInt(code) === 0) {
				...
				if (result === '9000') {
					...
					this.confirm0015(r.data.apdu, '00', r.data.proListNo);
				} else {
					this.confirm0015(r.data.apdu, '01', r.data.proListNo);
					util.sendException2Server(`青海二发金溢：${JSON.stringify(this.data.info)}`, cosArr, code, res);
				}
			} else {
				this.confirm0015(r.data.apdu, '01', r.data.proListNo);
		...
		bleUtil.cosChannel('00', cosArr, (code, res) => {
			if (parseInt(code) === 0) {
				...
				if (result === '9000') {
					...
					this.confirm0016(r.data.apdu, '00', r.data.proListNo);
				} else {
					this.confirm0016(r.data.apdu, '01', r.data.proListNo);
					util.sendException2Server(`青海二发金溢：${JSON.stringify(this.data.info)}`, cosArr, code, res);
				}
			} else {
				this.confirm0016(r.data.apdu, '01', r.data.proListNo);
	...
	// 写入数据到obu
	writeDataToObuForActive (r) {
		let cosArr = [r.data.apdu + r.data.mac];
		bleUtil.gvEsamChannel('20', cosArr, (code, res) => {
			if (parseInt(code) === 0) {
				let result = res[0];
				if (result === '9000') {
					this.confirmObuActive('00', r.data.proListNo);
				} else {
					this.confirmObuActive('01', r.data.proListNo);
					util.sendException2Server(`青海二发金溢：${JSON.stringify(this.data.info)}`, cosArr, code, res);
				}
			} else {
				this.confirmObuActive('01', r.data.proListNo);
