...
Page({
	...
	async onLoad (options) {
		console.log('套餐参数：',options);
		app.globalData.isTelemarketing = false;
		this.setData({
			contractStatus: +options.contractStatus
		});
		if (options.emptyPackageId) {	// 业务员空发订单
			let that = this;
			setTimeout(async () => {
				await that.getProduct(options.emptyPackageId);
			},1000);
			return;
		}
		if (!options.type) {
	...
	async next () {
		...
		if (this.data.listOfPackages[this.data.choiceIndex].shopProductId === app.globalData.emptyPackageId) {
			this.emptySaveOrder();
			return;
		}
	...
	async saveOrderInfo () {
		...
		if (!res) return;
		this.setData({isRequest: false});
		console.log('套餐3：',this.data.listOfPackages[this.data.choiceIndex]);
		let params = {
			orderId: app.globalData.orderInfo.orderId, // 订单id
