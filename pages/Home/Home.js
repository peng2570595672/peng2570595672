...
Page({
	...
	adBanner (e) {
		...
		console.log(e.detail.current,'111');
	},
	// 轮播图自动轮播时触发
	swiperChange (e) {
		const current = e.detail.current; // 当前轮播图的索引
		if (this.data.imgList[current].jumpUrl === '平安测额' || this.data.imgList[current].jumpUrl === '/pages/personal_center/quotaLandingPage/quotaLandingPage') {
			this.useBriedPoint();
		}
	},
	useBriedPoint () {
		...
		util.buriedPoint(params, data => {
			this.setData({ shouldBindChange: false }); // 不再绑定事件
