...
function getStatus(orderInfo) {
	...
	if (orderInfo.obuStatus === 0 || orderInfo.obuStatus === 3 || orderInfo.obuStatus === 4 || (orderInfo.status === 1 && orderInfo.obuStatus === 2 && (orderInfo.obuCardType === 23 || orderInfo.obuCardType === 2))) {//补充河北交投换卡换签
		...
		if (orderInfo.orderType === 71 && app.globalData.shopIdList.lnmShopIdsAbove.includes(orderInfo.shopId)) {  //辽宁移动线上电商 & 线下订单
			return 36;
		}
		return 11; //  待激活
	}
...
async function getListOfPackages(orderInfo, regionCode, notList) {
	...
	const creditCardBank = list.filter(item => item.flowVersion === 9); // 信用卡申请套餐
...
async function getDataFromServersV2(path, params = {}, method = 'POST', isLoading = true) {
	...
	// 设置请求头
	obj.header = header;
	// console.log('请求路径:' + path, '请求参数:' + JSON.stringify(params));
	// 执行请求
	return await new Promise((resolve, reject) => {
		wx.request({
			...
			success: (res) => {
				// console.log(obj.url);
				// console.log('返回 - 成功 & 失败:' + JSON.stringify(res.data));
...
async function getUserIsVip() {
	...
	if (!result) return;
	if (result.code === 0) {
		app.globalData.need_filterBannerList = result.data.showCzd;
 ...
 * @returns 
 */
async function addHandleOrAgreementTip(that, params, pop, content, callBack) {
		...
		if (result.code === 0) {
			...
			// console.log(result.data[`${Object.keys(params)[1]}`]);
			if (!result.data[`${Object.keys(params)[1]}`]) {
				that.selectComponent('#popTipComp').showCountdownPopupBox({
					type: pop.type,
					title: pop.title,
					btnShadowHide: true,
					btnCancel: pop.btnCancel,
					btnconfirm: pop.btnconfirm,
					delayTime: pop.delayTime,
					content: content,
					all: pop?.all ? 1 : 0,
					callBack: async () => {
						// 弹窗更新
						showLoading();
						let res = await getDataFromServersV2('consumer/order/update-right-confirm-dialog', params);
						if (!res) return;
						if (res.code === 0) {
							callBack && callBack();
							wx.hideLoading();
